def respiratory_ordering(original_matrix, original_bvals, original_bvecs, zoom = 'ON', IntERACT_zoom = 'ON', organ = 'Liver', operation_type = 'Magnitude'):
    """
    ########## Definition Inputs ##################################################################################################################
    ...
    from   CarDpy.Tools.IntERACT         import INTERACT_GUI, execute_crop, next_slice, finish_program, update_plots                                #
    from   CarDpy.Data_Sorting.Diffusion import sorted2stacked, stacked2sorted                                                                      # Import sorted to stacked and stacked to sorted from CarDpy
    from   CarDpy.Diffusion.Denoising    import denoise                                                                                             #
...
#                entropy_image = entropy(denoised_matrix[y_start[slc]:y_end[slc], x_start[slc]:x_end[slc], slc, dif, avg] / denoised_matrix[:, :, slc, dif, avg].max(), disk(7))                             # Compute entropy image
    ...
    return [respiratory_matrix, respiratory_bvals, respiratory_bvecs]
