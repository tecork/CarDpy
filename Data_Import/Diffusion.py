def DCM_Reader(dcm_path, info = 'ON'):
    """
    ########## Definition Inputs ##################################################################################################################
    ...
    from mystic_mrpy.Data_Import.Diffusion import VendorHeaders
    #from VendorHeaders import VendorHeaders
    ...
    ########## Identify b values, b vectors, and matrix information from headers ###################################################################
    ...
    numRows       = 0                                                                                       # Initialize number of rows (x)
    for ii in range(len(dcmPath)):                                                                          # Iterate through DICOMs
        ...
        slc_loc[ii]    = Header['Slice Location']                                                               # Extract slice location
...
def VendorHeaders(dcm):
    ...
    if Vendor =='SIEMENS':                                                                                                                          # If the vendor is Siemens...
