def cDTI_recon(myocardial_mask, Eigenvectors, num_interp_points = 200, smoothness_level = 'Low', helix_angle_outlier_criteria = 1):
    """
    ########## Definition Inputs ##################################################################################################################
    ...
    import math
    import numpy as np
    from   CarDpy.Diffusion.cDTI import myocardial_mask_contour_extractor, angular_contour_organization, countour_BSpline, microstructure_angle_projections
    from   CarDpy.Diffusion.cDTI import helix_angle_filtering, myocardial_mask_contour_filler, myocardial_contour_extraction
...
#        [epicardial_contour_dilated,      endocardial_contour_dilated]          = myocardial_mask_contour_extractor(myocardial_mask_dialated, slc)
        [Epicardium_Contours_Dialated, Endocardium_Contours_Dialated, _]         = myocardial_contour_extraction(myocardial_mask_dialated, slc)
        ...
        if len(endocardial_cordinates_dilated[slc][0]) > 1:
            [epicardial_interpolated_x_dilated,  epicardial_interpolated_y_dilated]  = countour_BSpline(epicardial_cordinates_dilated[slc][0],
            ...
            [endocardial_interpolated_x_dilated, endocardial_interpolated_y_dilated] = countour_BSpline(endocardial_cordinates_dilated[slc][0],
...
#        [epicardial_contour,      endocardial_contour]           = myocardial_mask_contour_extractor(myocardial_mask, slc)
        [Epicardium_Contours, Endocardium_Contours, _]           = myocardial_contour_extraction(myocardial_mask, slc)
        ...
        if len(endocardial_cordinates[slc][0]) > 1:
            [epicardial_interpolated_x,  epicardial_interpolated_y]  = countour_BSpline(epicardial_cordinates[slc][0],
                                                                                        epicardial_cordinates[slc][1],
                                                                                        num_interp_points,
                                                                                        smoothness_level)
            [endocardial_interpolated_x, endocardial_interpolated_y] = countour_BSpline(endocardial_cordinates[slc][0],
    ...
    helix_angle_filtered = np.empty(myocardial_mask.shape)
    helix_angle_filtered.fill(np.nan)                                                                                                                                # Fill helix angle matrix with np.nan
    ...
    for slc in range(slices):
        if (epicardial_cordinates_interpolated[slc] == ['No Data'] or endocardial_cordinates_interpolated[slc] == ['No Data']):
            ...
            [Helix_Angle, E2_Angle, Transverse_Angle] = microstructure_angle_projections(interpolated_mask[:, :, slc],
                                                                                         ...
                                                                                         points_epicardium,
                                                                                         points_endocardium)
            [Helix_Angle_Filtered]                    = helix_angle_filtering(Helix_Angle,
                                                                              points_epicardium,
                                                                              points_endocardium,
                                                                              outlier_standard_deviation = helix_angle_outlier_criteria)
            helix_angle[:, :, slc]          = Helix_Angle
            helix_angle_filtered[:, :, slc] = Helix_Angle_Filtered
    ...
    Cardiac_DTI_Metrics['HAF'] = helix_angle_filtered
...
def Spatial_Helix_Angle_Filtering(helix_angle, mask, wall_depth_factor = 0.25, kernel_size = 5):
    from   CarDpy.Tools.Contours import Myocardial_Mask_Contour_Extractor, Moving_Window_2D
    from   CarDpy.Tools.Contours import Comprehensive_Outter_Contour
    import math
    import pandas as pd
    ...
    spatial_helix_angle_filter = np.copy(helix_angle_original[:, :, slc])
    ###
    [epicardial_contour,      endocardial_contour]    = Myocardial_Mask_Contour_Extractor(mask, slc)
    ...
    for depth in range(len(concentric_contours_list)):
        ...
        for index in range(x_original.shape[0]):
            ...
            helix_angle[index]   = helix_angle_original[x_original[index], y_original[index], slc]
        ...
        helix_angle_updated = np.copy(helix_angle_original[:, :, slc])
        ...
        counter     = 0
        spatial_helix_angle_filter  = np.copy(HA_updated)
        temp_matrix = np.copy(HA_updated)
        for index in range(sorted_data_stacked.shape[0]):
